...
# search tne input shapefile port for the first shapefile
my_shape = glob.glob1(shape_path, '*.shp')[0]
# search the input image port for the first geotiff
my_image = glob.glob1(image_path, '*.tif')[0]

# define the name of the output data port
out_path = '/mnt/work/output/data_out'
# while the input ports are created by GBDX, you will need to
